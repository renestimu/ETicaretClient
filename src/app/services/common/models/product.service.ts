...
export class ProductService {
  ...
  async readList(
    ...
    proimiseData
      ...
      .catch((e: HttpErrorResponse) => errorCallBack(e.message));
  ...
  async readImages(
    ...
    const getO: Observable<List_Product_Image[]> = this.httpClientService.get<
      List_Product_Image[]
    >(
      {
        action: 'getimages',
  ...
  async updateStock(productId: string, stock: number, successCalback?: () => void, errorCallBack?: (errorMessage: string) => void) {
    const updateStockObservable = this.httpClientService.put(
      {
        controller: 'products',
        action: 'qrcode',
      },
    ...
    await firstValueFrom(updateStockObservable);
    successCalback();
  }
}
